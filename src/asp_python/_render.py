...
if TYPE_CHECKING:
    ...
    from ._model import AspPythonFinding, AspPythonReport
...
def _render_ok_header(report: AspPythonReport) -> str:
    project_root = _report_project_root(report)
    target = ", ".join(
        _render_display_path(path, project_root=project_root)
        for path in report.root_paths
...
def _software_criterion_labels(finding: AspPythonFinding) -> str:
...
def _reasoning_tree_import_roots(report: AspPythonReport) -> tuple[Path | str, ...]:
    if report.project_resolution is None:
...
def _report_project_root(report: AspPythonReport) -> Path | None:
    if report.project_resolution is None:
        return None
...
def _deduplicate_advice_findings(
    advice_findings: tuple[AspPythonFinding, ...],
    *,
    blocking_findings: tuple[AspPythonFinding, ...],
...
def _finding_key(finding: AspPythonFinding) -> tuple[str, str | None, int, int]:
    return (
        finding.rule_id,
