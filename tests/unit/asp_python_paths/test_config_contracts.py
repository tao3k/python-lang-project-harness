...
def test_default_asp_python_config_uses_default_rule_packs() -> None:
    config = default_asp_python_config()
...
def test_rule_pack_descriptors_and_json_renderer_are_stable(tmp_path: Path) -> None:
    ...
    payload = json.loads(render_asp_python_report_json(report))
