...
def test_run_asp_python_paths_collects_parse_findings(tmp_path: Path) -> None:
    ...
    report = run_asp_python_paths([tmp_path])
...
def test_run_asp_python_paths_uses_configured_discovery(tmp_path: Path) -> None:
...
def test_run_asp_python_paths_uses_configured_blocking_severities(
    tmp_path: Path,
) -> None:
    ...
    report = run_asp_python_paths([source], config=config)
    ...
    assert render_asp_python_report(report).startswith("[ok]")
