...
def test_assert_asp_python_clean_blocks_for_pytest(tmp_path: Path) -> None:
    ...
    try:
        ...
        raise AssertionError("ASP Python should block package warnings")
...
def test_asp_python_blocks_root_pytest_files(tmp_path: Path) -> None:
    tests = tmp_path / "tests"
    tests.mkdir()
    ...
    try:
        ...
        raise AssertionError("ASP Python should block root pytest files")

    assert "rule=PY-TEST-R001 severity=warning" in message
    assert "|message Pytest file is scattered in tests root" in message
    assert "tests/unit/" in message
    assert "tests/test_scattered.py" in message
    assert str(source) not in message


def test_asp_python_blocks_unexpected_tests_root_entries(tmp_path: Path) -> None:
...
def test_asp_python_blocks_bloated_unit_test_leaf(tmp_path: Path) -> None:
