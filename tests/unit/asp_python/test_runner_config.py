...
def test_runner_rejects_missing_project_root_and_explicit_path(tmp_path: Path) -> None:
    ...
    try:
        run_asp_python_paths([missing])
    except ValueError as error:
        assert str(error) == f"ASP Python path does not exist: {missing}"
    else:
        raise AssertionError("missing ASP Python path should fail")
...
def test_concurrent_parser_retains_deterministic_discovery_order(
    ...
    report = run_asp_python_paths([tmp_path], rule_packs=())
