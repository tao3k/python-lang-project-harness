...
def test_asp_python_policy_does_not_parse_python_source_directly() -> None:
    asp_python_sources = sorted(
        (_PROJECT_ROOT / "src" / "asp_python").glob("*_policy*.py")
    )

    for path in asp_python_sources:
...
def test_asp_python_semantic_roles_use_parser_symbol_helpers() -> None:
    asp_python_sources = sorted((_PROJECT_ROOT / "src" / "asp_python").rglob("*.py"))
    for path in asp_python_sources:
...
def test_asp_python_pyproject_metadata_comes_from_parser_boundary() -> None:
    asp_python_sources = sorted((_PROJECT_ROOT / "src" / "asp_python").rglob("*.py"))
