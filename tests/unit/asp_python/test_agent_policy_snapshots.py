...
def _assert_lang_snapshot(
    ...
    report = run_asp_python_paths(paths)
...
def _assert_filtered_snapshot(
    ...
    rendered = normalize_temp_root(render_asp_python_report(filtered), root)
    assert_snapshot(
        f"unit_test__agent_policy_snapshot__{snapshot_name}",
        rendered,
        source="tests/unit/asp_python/test_agent_policy_snapshots.py",
    )
